...
Partial Class Main
    ...
    Public isCherryTrail As Boolean = DetectCherryTrail()
    ...
    Private Function DetectCherryTrail() As Boolean
        Dim win32_baseboard As New ManagementObjectSearcher("root/CIMV2", "SELECT * FROM Win32_BaseBoard")
        Dim baseBoard As String = ""
        For Each board As ManagementObject In win32_baseboard.Get()
            baseBoard = board("Manufacturer").ToString.Trim & " " & board("Product").ToString.Trim
        Next
        Return baseBoard.Contains("Cherry Trail")
    End Function
    ...
    Private Sub CherryTrail()
        Dim prompt As New Console
        PrintDebug("*** Detected Cherry Trail base board ***", False, True)
        PrintDebug("*** Searching devices ***", False, True)
        PrintDebug("Adding internal device...")
        ReDim Preserve atadevices(0)
        atadevices(0) = "/dev/sda -d ata"
        ...
        If isadmin Then
            ...
            For i As Integer = 0 To usbscan.Count - 2 'the last item is an empty line
                If InStr(usbscan(i), "/dev/sd") < InStr(usbscan(i), "#") And InStr(usbscan(i), "/dev/sd") >= 0 Then
                    ...
                    usbdevices(i) = usbdev(0).Replace(vbCr, "").Replace(vbLf, "").Trim.Replace("-d scsi", "-d sat")
                End If
            Next
        End If
    End Sub

    Private Sub NormalBaseBoard()
        Dim prompt As New Console
        PrintDebug("*** Searching devices ***", False, True)
        ...
        If AppSettings.Detection.Csmi Then
            Dim csmiscan() As String = prompt.Smartctl("--scan -d csmi")
        ...
        Dim scan() As String = prompt.Smartctl("--scan -d ata")
        ...
        Dim satscan() As String = prompt.Smartctl("--scan -d sat")
                ...
                Else
                    ...
                    For c As Short = 0 To raidcontrollers.Count - 1
                        If raid.ControllerName(i).Contains(raidcontrollers(c).Name) Then
                            ...
                            Dim raidscan() As String = prompt.Smartctl("--scan -d scsi")
        ...
        If isunknown And IsNothing(csmidevices) And raid.ControllersCount > 0 Then
            ...
            Dim scsiscan() As String = prompt.Smartctl("--scan -d scsi")
    ...
    Private Sub CollectDevices()
        ...
        If Not IsNothing(raiddevices) Then
            ...
            For d As Integer = 0 To raiddevices.Count - 1
                ...
                If raiddevices(d).Contains("-d scsi") Then
                    ...
                    If Not isduplicate And Not raiddevice.IsOptical And
                        Not raiddevice.Scsi.Info.Product.Contains("COVECUBECoveFs") And raiddevice.IsValid Then
                ...
                Else
                    ...
                    If Not isduplicate And Not raiddevice.IsOptical And
                        Not raiddevice.ATA.Info.Model.Contains("COVECUBECoveFs") And raiddevice.IsValid Then
                        devlist.Add(raiddevice)
                    End If
                    raiddevice = Nothing
                End If
        ...
        If Not IsNothing(scsidevices) Then
            ...
            For d As Integer = 0 To scsidevices.Count - 1
                ...
                If scsidevice.Scsi.IsScsiDisk And Not scsidevice.Scsi.Info.Product.Contains("COVECUBECoveFs") And Not _
                         scsidevice.Scsi.Info.Vendor.Contains("Areca") And Not _
                         scsidevice.Scsi.Info.Product.Contains("Raid") Then
        ...
        If Not IsNothing(usbdevices) Then
            ...
            For d As Integer = 0 To usbdevices.Count - 1
                If Not IsNothing(usbdevices(d)) Then
                    ...
                    Dim usb As New Device(usbdevices(d), DeviceType.USB, False)
                    ...
                    Else
                        ...
                        For i As Short = 0 To devlist.Count - 1
                            If devlist(i).ATA.Info.Model = usb.ATA.Info.Model And devlist(i).ATA.Info.SerialNumber = usb.ATA.Info.SerialNumber Then
                                addIt = False
                                Exit For
                            End If
                        Next
                        If usb.IsValid And addIt Then devlist.Add(New Device(usbdevices(d), DeviceType.USB))
                        usb = Nothing
                    End If
                    usb = Nothing
                End If
        ...
        If AppSettings.Detection.Add3ware Then
        ...
        With lvwDevices
            ...
            For Each dev As Device In devlist
                ...
                ElseIf dev.IsVirtual Or dev.Type = DeviceType.ThreeWare Then
                    If dev.Type = DeviceType.ATA Or dev.Type = DeviceType.ThreeWare Then
    ...
    Public Sub OverviewUpdate(ByVal i As Integer)
        With lvwDevices
            ...
            If IsNumeric(devlist(i).Temperature) Then
                ...
                Dim limit As Integer = 55
                If devlist(i).Type <> DeviceType.SCSI Then
                    If IsNumeric(devlist(i).ATA.Temperatures.RecommendedMax) Then
                        limit = devlist(i).ATA.Temperatures.RecommendedMax
                    Else
                        limit = 55
                    End If
                End If
