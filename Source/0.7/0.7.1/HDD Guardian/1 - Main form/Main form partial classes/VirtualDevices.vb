...
Partial Class Main
    ...
    Public Sub AddVirtual(ByVal description As String, ByVal path As String)
        ...
        If add Then
            ...
            IO.File.Copy(path, My.Application.Info.DirectoryPath & "\Data\" & My.User.Name & "\" & fname, True)
            'add virtual device to devices listview and collection
            devlist.Add(New Device(path, DeviceType.ATA, False, True, description))
    ...
    Public Sub RemoveVirtual(ByVal path As String)
        ...
        For i As Short = 0 To VirtualDevices.Count - 1
            If VirtualDevices(i).Path = path Then
                ...
                Dim fname As String = IO.Path.GetFileName(path)
                IO.File.Delete(My.Application.Info.DirectoryPath & "\Data\" & My.User.Name & "\" & fname)
