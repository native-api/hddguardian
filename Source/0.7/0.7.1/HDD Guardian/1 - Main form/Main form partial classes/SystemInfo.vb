...
Public Structure Drive
    ...
    Dim Caption, Model, Index, NumberOfPartitons, Size, SerialNumber As String
...
Public Structure Hardware
    Dim Bios, Cpu, Ram, Motherboard, VideoCard As String
End Structure
...
Partial Class Main
    ...
    Public Sub AnalyzeWMI()
        If Not drivesBGWorker.IsBusy Then drivesBGWorker.RunWorkerAsync()
    End Sub
                    ...
                    For l As Short = 0 To Drives(d).Partitions(p).Disk.Count - 1
                        With .Nodes("root").Nodes("d" & d).Nodes("p" & p)
                            If Drives(d).Partitions(p).Disk(l).VolumeName.Length > 0 Then
                                .Nodes.Add("l" & l, Drives(d).Partitions(p).Disk(l).VolumeName & " (" & Drives(d).Partitions(p).Disk(l).VolumeLetter & ")", 1, 1)
                                .Nodes(.Nodes.Count - 1).Tag = Drives(d).Index
                            Else
                                .Nodes.Add("l" & l, "(" & Drives(d).Partitions(p).Disk(l).VolumeLetter & ")", 1, 1)
                                .Nodes(.Nodes.Count - 1).Tag = Drives(d).Index
                            End If
                    ...
                    With Me
                        Select Case AppSettings.General.ComputerType
                            ...
                            Case Else
                                .picHardware.Image = My.Resources.pc_office
                        End Select
                        .lblHrwBiosValue.Text = Hrw.Bios
                        .lblHrwMotherboardValue.Text = Hrw.Motherboard
                        .lblHrwProcessorValue.Text = Hrw.Cpu
                        .lblHrwRamValue.Text = Hrw.Ram
                        .lblHrwVideoCardValue.Text = Hrw.VideoCard
                    End With
                    htcSystem.SelectPage(tpHardware)
                Case 2
                    For i As Short = 0 To Drives.Count - 1
                        If Drives(i).Model = path(1) And Drives(i).Index = .SelectedNode.Tag Then
                            With Me
                                .lblDiskSNValue.Text = Drives(i).SerialNumber
                                .lblDiskIndexValue.Text = Drives(i).Index
                                .lblDiskPartitionsValue.Text = Drives(i).NumberOfPartitons
                                .lblDiskSizeValue.Text = Drives(i).Size
                            End With
                            Exit For
                        End If
                    Next
                    htcSystem.SelectPage(tpDisk)
                Case 3
                    For i As Short = 0 To Drives.Count - 1
                        If Drives(i).Model = path(1) And Drives(i).Index = .SelectedNode.Tag Then
                            For p As Short = 0 To Drives(i).Partitions.Count - 1
                                If Drives(i).Partitions(p).DeviceID.Replace("#", "").Contains(path(2)) Then
                                    With Me
                                        .lblBootableValue.Text = Drives(i).Partitions(p).Bootable
                                        .lblBootPartitionValue.Text = Drives(i).Partitions(p).BootPartition
                                        .lblPrimaryPartitionValue.Text = Drives(i).Partitions(p).PrimaryPartition
                                        .lblPartitionTypeValue.Text = Drives(i).Partitions(p).Description
                                        .lblBlocksValue.Text = Drives(i).Partitions(p).NumberOfBlocks
                                        .lblBlockSizeValue.Text = Drives(i).Partitions(p).BlockSize
                                        .lblPartitionSizeValue.Text = Drives(i).Partitions(p).Size
                                    End With
                                    Exit For
                                End If
                            Next
                        ...
                        End If
                    Next
                    htcSystem.SelectPage(tpPartition)
                Case 4
                    For i As Short = 0 To Drives.Count - 1
                        If Drives(i).Model = path(1) And Drives(i).Index = .SelectedNode.Tag Then
                            For p As Short = 0 To Drives(i).Partitions.Count - 1
                                If Drives(i).Partitions(p).DeviceID.Replace("#", "").Contains(path(2)) Then
                                    For l As Short = 0 To Drives(i).Partitions(p).Disk.Count - 1
                                        If path(3).Contains(Drives(i).Partitions(p).Disk(l).VolumeLetter) Then
                                            Dim di As New IO.DriveInfo(Drives(i).Partitions(p).Disk(l).VolumeLetter.Replace(":", ""))
                                            If di.AvailableFreeSpace < 50 * 1024 * 1024 Then
                                                tlpBenchmark.Visible = False
                                                tlpNoBenchmark.Visible = True
                                            Else
                                                tlpBenchmark.Visible = True
                                                tlpNoBenchmark.Visible = False
                                            End If
                                            If Not BenchmarkBGWorker.IsBusy Then
                                                diskToBench = Drives(i).Partitions(p).Disk(l).VolumeLetter
                                                partitionSerial = Drives(i).Partitions(p).Disk(l).SerialNumber
                                            End If
                                            tlpBenchInfo.Visible = BenchmarkBGWorker.IsBusy
                                            PopulateBench()
                                            With Me
                                                .lblLogicalSNValue.Text = Drives(i).Partitions(p).Disk(l).SerialNumber
                                                .lblFileSystemValue.Text = Drives(i).Partitions(p).Disk(l).FileSystem
                                                .lblLogicalSizeValue.Text = Drives(i).Partitions(p).Disk(l).TotalSize
                                            End With
                                            Exit For
                                        End If
                                    Next
                        ...
                        End If
                    Next
                    htcSystem.SelectPage(tpLogicalDisk)
            End Select
    ...
    Private Sub GetHardwareData()
        ...
        '║        Win32_BIOS            ║
        '╚══════════════════════════════╝
        Dim win32_bios As New ManagementObjectSearcher("root/CIMV2", "SELECT * FROM Win32_BIOS")
        For Each bios As ManagementObject In win32_bios.Get()
            Hrw.Bios = bios("Manufacturer").ToString.Trim & " " & bios("Version").ToString.Trim
        Next
        win32_bios = Nothing
        ...
        For Each cpu As ManagementObject In win32_processor.Get()
            Hrw.Cpu = cpu("Name").ToString.Trim & vbCrLf _
                & cpu("Version").ToString.Trim & vbCrLf _
                & cpu("CurrentClockSpeed").ToString.Trim & " MHz" & vbCrLf _
                & cpu("SocketDesignation").ToString.Trim
        Next
        win32_processor = Nothing
        ...
        For Each bank As ManagementObject In win32_physicalmemory.Get()
            banks += bank("BankLabel") & ": " & Format(bank("Capacity") / 1024 ^ 2, "#,###") & " MiB " &
                bank("Manufacturer").ToString.Trim & " " & bank("PartNumber").ToString.Trim & vbCrLf

            'totalsize += bank("Capacity")
        Next
        win32_physicalmemory = Nothing

        Hrw.Ram = banks.Remove(banks.Length - 2, 2) 'Format(totalsize / 1024 ^ 2, "#,###") & " MiB"
        ...
        For Each card As ManagementObject In win32_videocontroller.Get()
            cards += card("Name").ToString.Trim & vbCrLf
        Next
        win32_videocontroller = Nothing
        Hrw.VideoCard = cards.Remove(cards.Length - 2, 2)
    ...
    Private Sub GetData()
        ...
        For Each diskdrive As ManagementObject In win32_diskdrive.Get()
            ...
            d.SerialNumber = diskdrive("SerialNumber").ToString.Trim
            ...
            For Each diskpartition As ManagementObject In win32_diskpartition.Get()
                ...
                For Each logicaldisktopartition As ManagementObject In win32_logicaldisktopartition.Get
                    ...
                    If antecedent.Contains(dev_id) Then
                        ...
                        For Each logicaldisk As ManagementObject In win32_logicaldisk.Get
                            ...
                            'here we avoid drive duplicates
                            Dim toAdd As Boolean = True
                            If Not IsNothing(p.Disk) Then
                                For ild As Short = 0 To p.Disk.Count - 1
                                    If p.Disk(ild).VolumeLetter = ld.VolumeLetter Then
                                        toAdd = False
                                        Exit For
                                    End If
                                Next
                ...
                d.Partitions.Add(p)
                p = Nothing
                win32_logicaldisktopartition = Nothing
            Next
            win32_diskpartition = Nothing
