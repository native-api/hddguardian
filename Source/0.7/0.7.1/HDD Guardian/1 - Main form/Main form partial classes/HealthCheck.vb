            ...
            For Each Device In devlist
                ...
                If IsNumeric(Device.Temperature) Then
                    If Convert.ToSingle(Device.Temperature) > tmax And Not Device.MailNotification.SentTemperature Then
                        If Device.Type = DeviceType.SCSI Then
                            threshold += Device.Scsi.Info.Product & " (" & Device.Location & ")" & vbCrLf
                        Else
                            threshold += Device.ATA.Info.Model & " (" & Device.Location & ")" & vbCrLf
                        End If
                    End If
                End If
            Next
    ...
    Public Sub PowerOff()
        ...
        If deviceFailure Then
            ...
            If System.Environment.OSVersion.Version.Major >= 6 And My.User.IsInRole(ApplicationServices.BuiltInRole.Administrator) Then
                ...
                If AppSettings.Warnings.ForceClose Then
                    c.Shutdown("/s /t " & t & " /f /c ""HDD Guardian: device failure"" /d u:1:7")
                Else
                    c.Shutdown("/s /t " & t & " /c ""HDD Guardian: device failure"" /d u:1:7")
                End If
            Else
                If AppSettings.Warnings.ForceClose Then
                    c.Shutdown("-s -t " & t & " -f -c ""HDD Guardian: device failure"" -d u:1:7")
        ...
        If highTemp Then
            ...
            If System.Environment.OSVersion.Version.Major >= 6 And My.User.IsInRole(ApplicationServices.BuiltInRole.Administrator) Then
                'Vista or higher
                If AppSettings.Warnings.ForceClose Then
                    c.Shutdown("/s /t " & t & " /f /c ""HDD Guardian: excessive temperature"" /d u:1:7")
                Else
                    c.Shutdown("/s /t " & t & " /c ""HDD Guardian: excessive temperature"" /d u:1:7")
                End If
            Else
                If AppSettings.Warnings.ForceClose Then
                    c.Shutdown("-s -t " & t & " -f -c ""HDD Guardian: excessive temperature"" -d u:1:7")
