...
'
'Copyright (C) 2014-2016 Samuele Parise
'
    ...
    Private Sub SmartctlScan()
        ...
        With txtConsole
            ...
            If My.User.IsInRole(ApplicationServices.BuiltInRole.Administrator) Then
                .AppendText("=== HDD Guardian Toolbox | Administrator ===" & vbCrLf)
            Else
                .AppendText("=== HDD Guardian Toolbox | Regular user ===" & vbCrLf)
            End If
            .AppendText(vbCrLf)
            ...
            If Not IsNothing(int_devices) Then
                ...
                For i As Short = 0 To int_devices.Length - 1
                    ...
                    .AppendText("smartctl -x -g wcreorder " & int_devices(i) & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText(prompt.SendCmd("-x -g wcreorder " & int_devices(i)))
                    .AppendText(vbCrLf)
                Next i
            Else
                .AppendText("=== ATA: no device detected ===" & vbCrLf)
                .AppendText(vbCrLf)
            End If
            ...
            If Not IsNothing(ext_devices) Then
                ...
                For i As Short = 0 To ext_devices.Length - 1
                    ...
                    .AppendText("=== Scan device " & name(0) & " ===" & vbCrLf)
                    .AppendText("smartctl -x -g wcreorder " & ext_devices(i) & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText(prompt.SendCmd("-x -g wcreorder " & ext_devices(i)))
                    .AppendText(vbCrLf)
                Next i

            Else
                .AppendText("=== USB: no device detected ===" & vbCrLf)
                .AppendText(vbCrLf)
            End If
            ...
            If Not IsNothing(scsi_devices) Then
                ...
                For i As Short = 0 To scsi_devices.Length - 1
                    ...
                    .AppendText("=== Scan device " & name(0) & " ===" & vbCrLf)
                    .AppendText("smartctl -x -g all " & scsi_devices(i) & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText(prompt.SendCmd("-x -g all " & scsi_devices(i)))
                    .AppendText(vbCrLf)
                Next i

            Else
                .AppendText("=== SCSI: no device detected ===" & vbCrLf)
                .AppendText(vbCrLf)
            End If
            ...
            If Not IsNothing(csmi_devices) Then
                ...
                For i As Short = 0 To csmi_devices.Length - 1
                    ...
                    .AppendText("=== Scan device " & name(0) & " ===" & vbCrLf)
                    .AppendText("smartctl -x -g wcreorder " & csmi_devices(i) & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText(prompt.SendCmd("-x -g wcreorder " & csmi_devices(i)))
                    .AppendText(vbCrLf)
                Next i

            Else
                .AppendText("=== Intel Matrix: no device detected ===" & vbCrLf)
                .AppendText(vbCrLf)
            End If

            .AppendText("╔═════════════════════════════════╗" & vbCrLf)
            .AppendText("║  Scan for NVM Express devices   ║" & vbCrLf)
            .AppendText("╚═════════════════════════════════╝" & vbCrLf)
            .AppendText("smartctl --scan -d nvme" & vbCrLf)
            .AppendText(vbCrLf)


            Dim nvme As String = prompt.SendCmd("--scan -d nvme")
            .AppendText(nvme)
            .AppendText(vbCrLf)

            Dim nvme_devices() As String = Nothing
            Dim nvme_arr() As String = nvme.Split(vbCrLf)
            Dim dn As Integer = -1
            For i As Integer = 0 To nvme_arr.Length - 2
                If nvme_arr(i).Contains("#") Then
                    dn += 1
                    ReDim Preserve nvme_devices(dc)
                    Dim dev As String() = nvme_arr(i).Split("#")
                    nvme_devices(dc) = dev(0).Trim
                End If
            Next

            If Not IsNothing(nvme_devices) Then
                .AppendText("=== " & nvme_devices.Count & " Intel Matrix connected devices detected ===" & vbCrLf)
                .AppendText(vbCrLf)

                For i As Short = 0 To nvme_devices.Length - 1
                    Dim name() As String = nvme_devices(i).Split(" ")
                    .AppendText("=== Scan device " & name(0) & " ===" & vbCrLf)
                    .AppendText("smartctl -x -g wcreorder " & nvme_devices(i) & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText(prompt.SendCmd("-x -g wcreorder " & nvme_devices(i)))
                    .AppendText(vbCrLf)
                Next i

            Else
                .AppendText("=== NVM Express: no device detected ===" & vbCrLf)
                .AppendText(vbCrLf)
            End If
        End With
    ...
    Private Sub UsbScanner()
        ...
        With txtUsb
            ...
            For i As Short = 97 To 122
                ...
                Dim unable As Boolean = False

                unknown = scan.Contains("Unknown USB bridge")
                unable = scan.Contains("Unable to read USB device ID")

                If unknown Or unable Then
                    If unknown Then .AppendText(">>>>>>>> Unknown USB bridge detected <<<<<<<<" & vbCrLf)
                    If unable Then .AppendText(">>>>>>>> Try to detect USB bridge <<<<<<<<" & vbCrLf)
                    .AppendText(vbCrLf)
                    .AppendText("******** Try to treat /dev/sd" & Chr(i) & " as SCSI to ATA (SAT) ********" & vbCrLf)
                    .AppendText(vbCrLf)
                    Dim sat As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d sat")
                    .AppendText(sat)
                    .AppendText(vbCrLf)
                    Dim sat12 As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d sat,12")
                    .AppendText(sat12)
                    .AppendText(vbCrLf)
                    .AppendText("******** Try to treat /dev/sd" & Chr(i) & " as UsbCypress ********" & vbCrLf)
                    .AppendText(vbCrLf)
                    Dim cypress As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbcypress")
                    .AppendText(cypress)
                    .AppendText(vbCrLf)
                    .AppendText("******** Try to treat /dev/sd" & Chr(i) & " as UsbSunPlus ********" & vbCrLf)
                    .AppendText(vbCrLf)
                    Dim sunplus As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbsunplus")
                    .AppendText(sunplus)
                    .AppendText(vbCrLf)
                    .AppendText("******** Try to treat /dev/sd" & Chr(i) & " as Prolific ********" & vbCrLf)
                    .AppendText(vbCrLf)
                    Dim prolific As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbprolific")
                    .AppendText(prolific)
                    .AppendText(vbCrLf)
                    .AppendText("******** Try to treat /dev/sd" & Chr(i) & " as UsbJMicron ********" & vbCrLf)
                    .AppendText(vbCrLf)
                    Dim jmicron As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbjmicron")
                    .AppendText(jmicron)
                    .AppendText(vbCrLf)
                    Dim jmicron0 As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbjmicron,0")
                    .AppendText(jmicron0)
                    .AppendText(vbCrLf)
                    Dim jmicron1 As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbjmicron,1")
                    .AppendText(jmicron1)
                    .AppendText(vbCrLf)
                    Dim jmicronp As String = prompt.SendCmd("-x /dev/sd" & Chr(i) & " -d usbjmicron,p")
                    .AppendText(jmicronp)
                    .AppendText(vbCrLf)
                End If
            Next
