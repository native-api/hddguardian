...
#Region "ATA variables"
    ...
    Private ataAam, ataRecommendedAam, ataApm, ataStandby As Short
    ...
    Private ataBadSectorsThreshold As String
...
#Region "ATA data processors"
    ...
    Private Sub GetDeviceInfo()
        If devOutput.Length > 0 Then
            ...
            ataDeviceModel = GetInfo("Device Model")
    ...
    Private Sub Reliability()
        ...
        'atarating.Errors = Math.Round(5 - r_e / 20)
        'If atarating.Errors < 1 Then atarating.Errors = 1

        If Not IsNothing(id5) Then
            r_5 = Math.Round(id5 / ataBadSectorsThreshold * 100)
        ...
        With ataNewVitals
            If ataIsSsd Then ataRating.Overall = 0
        ...
        If devIsValid = False Then ataRating.Overall = 0
    End Sub
    ...
    Private Sub GetAttributes()
        ...
        Dim matchOld As String = "[0-9]{1,3}\s+[a-zA-Z0-9_ -]{23}\s+[-+POSRCK]{6}\s+[0-9]{3}\s+[0-9]{3}\s+[0-9]{3}\s+(Pre-fail|Old_age)\s+(Always|Offline)\s+(-|FAILING_NOW|In_the_past)\s+.*"
        ...
        Dim matchBrief As String = "[0-9]{1,3}\s+[a-zA-Z0-9_ -]{23}\s+[-+POSRCK]{6}\s+[0-9]{3}\s+[0-9]{3}\s+[0-9]{3}\s+(-|NOW|Past)\s+.*"
            ...
            For ri As Integer = i + 1 To devOutput.Length - 1
                ...
                If Not devOutput(ri).Contains("<==") Then
                    ...
                    If devOutput(ri).Contains("5 Reallocated_Sector_Ct") Or devOutput(ri).Contains("5 Runtime_Bad_Block") Then
                        ataBadSectorsThreshold = GetThresholdValue(attr)
                    ...
                    If devOutput(ri).Contains("233 Media_Wearout_Indicator") Then
                        ...
                        ataIsSsd = True
                    End If
                    If devOutput(ri).Contains("177 Wear_Leveling_Count") And devOutput(ri).Contains("Always") Then
    ...
    Private Sub GetErrors()
        ...
        Else
            ...
            For i As Long = last To last - 4 Step -1
                ...
                If r > 0 Then
                    ...
                    'set registers
                    Dim regRow As Integer = GetIndexOf("--", r) + 1
                    Dim rev As New AtaRegister(devOutput(regRow).Trim.Split(" ", 8, StringSplitOptions.RemoveEmptyEntries))

                    'set commands
                    Dim cmds As New List(Of AtaCommand)
                    cmds.Clear()
                    Dim cmdRow As Integer = GetIndexOf("--", regRow) + 1
                    For cmdIndex As Integer = cmdRow To cmdRow + 4
                        Dim cmd() As String = devOutput(cmdIndex).Trim.Split(" ", 10, StringSplitOptions.RemoveEmptyEntries)
                        If cmd.Length > 0 Then
                            cmds.Add(New AtaCommand(cmd))
                        Else
                            Exit For
                        End If
                    Next

                    Dim err As New AtaError(i, lt(1).Trim, status(1).Replace("the device was", "").Replace(".", "").Trim, rev, cmds)
        ...
        'atanewvitals.Errors = m_totalerrors
    End Sub
...
#Region "ATA classes"
    ...
    Public Class AtaInfo
        ...
        Public ReadOnly Property Model As String
            Get
                If pd.devType = DeviceType.ThreeWare And Not pd.devIsValid Then Return "3ware clipboard"
    ...
    Public Class AtaFeatures
        ...
        Public Class AtaAam
            ...
            Public Property Value As String
                ...
                Set(ByVal value As String)
                    pd.smartctl.SendCmd("-s aam," & value & " " & pd.devLocation)
                    pd.Update()
                End Set
            End Property
        ...
        Public Class AtaApm
            ...
            Public Property Value As String
                ...
                End Get
                Set(ByVal value As String)
                    pd.smartctl.SendCmd("-s apm," & value & " " & pd.devLocation)
                    pd.Update()
                End Set
            End Property
        ...
        Public Property Standby As String
            ...
            End Get
            Set(ByVal value As String)
                pd.smartctl.SendCmd("-s standby," & value & " " & pd.devLocation)
                pd.ataStandby = value
                pd.SaveSettings()
            End Set
        End Property
        ...
        Public Class AtaWriteCache
            ...
            Public Property Enabled As Feature
                ...
                Set(ByVal value As Feature)
                    If value = Feature.Enabled Then
                        pd.smartctl.SendCmd("-s wcache,on " & pd.devLocation)
                    ElseIf value = Feature.Disabled Then
                        pd.smartctl.SendCmd("-s wcache,off " & pd.devLocation)
                    End If
                    pd.Update()
                End Set
            End Property
            ...
            Public Property Reorder As Feature
                ...
                Set(ByVal value As Feature)
                    If value = Feature.Enabled Then
                        pd.smartctl.SendCmd("-s wcreorder,on " & pd.devLocation)
                    ElseIf value = Feature.Disabled Then
                        pd.smartctl.SendCmd("-s wcreorder,off " & pd.devLocation)
                    End If
                    pd.Update()
                End Set
            End Property
        ...
        Public Property LookAhead As Feature
            ...
            Set(ByVal value As Feature)
                If value = Feature.Enabled Then
                    pd.smartctl.SendCmd("-s lookahead,on " & pd.devLocation)
                ElseIf value = Feature.Disabled Then
                    pd.smartctl.SendCmd("-s lookahead,off " & pd.devLocation)
                End If
                pd.Update()
            End Set
        End Property
        ...
        Public Class AtaOfflineAuto
            ...
            Public Sub Enable()
                Dim feature() As String = pd.smartctl.SendCmd("-o on " & pd.devLocation)
            End Sub

            Public Sub Disable()
                Dim feature() As String = pd.smartctl.SendCmd("-o off " & pd.devLocation)
            End Sub
    ...
    Public Class AtaError
        ...
        Dim _register As AtaRegister
        Dim _commands As List(Of AtaCommand)

        Public Sub New(ByVal number As Integer, ByVal lifeTime As String, ByVal status As String, ByVal register As AtaRegister, ByVal commands As List(Of AtaCommand))
            _number = number
            _lifeTime = lifeTime
            _status = status
            _register = register
        ...
        Public ReadOnly Property Register As AtaRegister
            Get
                Return _register
            End Get
        End Property

        Public ReadOnly Property Commands As List(Of AtaCommand)
            Get
                Return _commands
            End Get
        End Property
    ...
    <System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)>
    Public Class AtaRegister
        Dim _er, _st, _sc, _sn, _cl, _ch, _dh, _error As String
        Public Sub New(ByVal register() As String)
            _er = register(0)
            _st = register(1)
            _sc = register(2)
            _sn = register(3)
        ...
        Public ReadOnly Property ER As String
            Get
                Return _er
            End Get
        End Property
        ...
        Public ReadOnly Property SC As String
            Get
                Return _sc
            End Get
        End Property
        ...
        Public ReadOnly Property CH As String
            Get
                Return _ch
            End Get
        End Property

        Public ReadOnly Property DH As String
            Get
                Return _dh
            End Get
        End Property
    ...
    Public Class AtaCommand
        Dim _cr, _fr, _sc, _sn, _cl, _ch, _dh, _dc, _powerUp, _feature As String
        ...
        Public ReadOnly Property PowerUp As String
            Get
                Return _powerUp
            End Get
        End Property
    ...
    Public Class AtaTest
        ...
        Public Sub Run(ByVal test As TestType)
            ...
            Select Case test
                Case TestType.Offline
                    pd.smartctl.SendCmd("-t offline " & pd.devLocation)
                    d = _polling.Offline
                Case TestType.Short
                    pd.smartctl.SendCmd("-t short " & pd.devLocation)
                    d = _polling.Short
                Case TestType.Extended
                    pd.smartctl.SendCmd("-t long " & pd.devLocation)
                    d = _polling.Extended
                Case TestType.Conveyance
                    pd.smartctl.SendCmd("-t conveyance " & pd.devLocation)
        ...
        Public Sub Abort()
            pd.smartctl.SendCmd("-X " & pd.devLocation)
            _timer.Stop()
            _percentage = 0
            _isRunning = False
            _endTest = Now
