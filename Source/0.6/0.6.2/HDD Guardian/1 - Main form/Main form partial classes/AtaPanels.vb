...
Partial Class Main
    ...
    Private Sub GeneralInfo()
        ...
        lblFormValue.Text = dev.FormFactor
        ...
        With lblErrorsAta
            If Not IsNothing(dev.Errors) Then
                Select Case Val(dev.TotalErrors)
                    ...
                    Case Is > 1
                        .ForeColor = status_warning
                        .Text = error_few.Replace("%", dev.TotalErrors)
                End Select
            Else
                .ForeColor = status_unknown
                .Text = "N/A"
        ...
        'work time
        lblAtaWorkTimeValue.Text = dev.WorkTime
        If dev.WorkTime <> "N/A" Then lblAtaWorkTimeValue.Text += vbCrLf & "(" & dev.WorkTimeHuman & ")"
        '****************************
        ...
        lnkUpdateDatabase.Visible = Not dev.InDatabase
        '****************
        '*** warnings ***
        '****************
    ...
    Private Sub AtaErrors()
        ...
        Else
            Dim err As Long = dev.TotalErrors
    ...
    Private Sub DisplayAtaError(ByVal e As Integer)
        ...
        If dev.Errors.Count > 0 Then
            If e > dev.TotalErrors Then Exit Sub

            Dim err As SmartError = dev.Errors(dev.TotalErrors - e)
            ...
            Dim wt() As String = dev.WorkTime.Split(" ")
            If wt(0) >= elt(0) Then
            ...
            If Not IsNothing(reg.ErrorText) Then
                Dim lba() As String = reg.ErrorText.Replace("at LBA =", "|").Split("|")
        ...
        Select Case cboTest.SelectedIndex
            Case 0
                ...
                If IsNumeric(t(0)) Then
                    Dim polltime As Long = Val(t(0))
                    Dim h As Long = Val(t(0)) \ 3600
                    Dim m As Long = (Val(t(0)) - h * 3600) \ 60
                    Dim s As Long = Val(t(0)) - h * 3600 - m * 60
            ...
            Case 1
                ...
                If IsNumeric(t(0)) Then
                    Dim polltime As Long = Val(t(0))
                    Dim h As Long = Val(t(0)) \ 60
                    Dim m As Long = Val(t(0)) - h * 60
        ...
        With cboDevices ' lvwDevices
            If Not IsNothing(.SelectedIndex) Then ' .SelectedItems.Count) Then
                ...
                Else
                    ...
                    Else
                        lblRating1Value.Text = d.NewVitalParameters.ReallSect & "/" & d.BadSectorsThreshold
                        With picRating1
                    ...
                    If Not IsNumeric(d.NewVitalParameters.ReallEvent) Then
                        lblRating2Value.Text = "N/A"
                        picRating2.Image = My.Resources.r0_star
                    Else
                        lblRating2Value.Text = d.NewVitalParameters.ReallEvent & "/" & My.Settings.Tune196
                        With picRating2
    ...
    Public Sub Temperatures()
        ...
        lblCycleMin.Text = CorF(dev.Temperatures.CycleMin)
        lblCycleMax.Text = CorF(dev.Temperatures.CycleMax)

        With tbCycle
            If IsNumeric(dev.Temperatures.CycleMin) Then
                .SetTemperatures(dev.Temperatures.CycleMin, dev.Temperatures.CycleMax)
            Else
                .SetTemperatures(0, 0)
            End If
        End With

        lblLifeTimeMin.Text = CorF(dev.Temperatures.LifetimeMin)
        lblLifeTimeMax.Text = CorF(dev.Temperatures.LifetimeMax)

        With tbLifeTime
            If IsNumeric(dev.Temperatures.LifetimeMin) Then
                .SetTemperatures(dev.Temperatures.LifetimeMin, dev.Temperatures.LifetimeMax)
            Else
                .SetTemperatures(0, 0)
            End If
        End With

        lblRecommendedMin.Text = CorF(dev.Temperatures.RecommendedMin)
        lblRecommendedMax.Text = CorF(dev.Temperatures.RecommendedMax)

        With tbRecommended
            If IsNumeric(dev.Temperatures.RecommendedMin) Then
                .SetTemperatures(dev.Temperatures.RecommendedMin, dev.Temperatures.RecommendedMax)
            Else
                .SetTemperatures(0, 0)
            End If
        End With

        lblLimitMin.Text = CorF(dev.Temperatures.LimitMin)
        lblLimitMax.Text = CorF(dev.Temperatures.LimitMax)

        With tbLimit
            If IsNumeric(dev.Temperatures.LimitMin) Then
                .SetTemperatures(dev.Temperatures.LimitMin, dev.Temperatures.LimitMax)
            Else
                .SetTemperatures(0, 0)
            End If
        End With
