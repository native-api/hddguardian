...
#End Region
        ...
        Public Sub SetIcon(ByVal value As String, ByVal health As Status, Optional ByVal displaylife As Boolean = False)
            ...
            If My.Settings.TempFahrenheit Then
                icnbmp = My.Resources.devices_tray_icons_F
            Else
                icnbmp = My.Resources.devices_tray_icons
            End If

            Select Case health
                Case Status.Failed
                    If displaylife Then
                        icnbmp = My.Resources.ssd_tray_icons
                        If IsNumeric(value) Then
                            Dim rect As New Rectangle(value * 16, 16, 16, 16)
                            Dim geticon As IntPtr = icnbmp.Clone(rect, icnbmp.PixelFormat).GetHicon
                            Dim icn As Icon = Icon.FromHandle(geticon)
                            ti.Icon = icn
                            DestroyIcon(icn.Handle)
                        Else
                            Dim rect As New Rectangle(101 * 16, 16, 16, 16)
                            Dim geticon As IntPtr = icnbmp.Clone(rect, icnbmp.PixelFormat).GetHicon
                            Dim icn As Icon = Icon.FromHandle(geticon)
                            ti.Icon = icn
                            DestroyIcon(icn.Handle)
                ...
                Case Status.Passed
                    If displaylife Then
                        icnbmp = My.Resources.ssd_tray_icons
                        If IsNumeric(value) Then
                            Dim rect As New Rectangle(value * 16, 0, 16, 16)
                            Dim geticon As IntPtr = icnbmp.Clone(rect, icnbmp.PixelFormat).GetHicon
                            Dim icn As Icon = Icon.FromHandle(geticon)
                            ti.Icon = icn
                            DestroyIcon(icn.Handle)
...
#End Region
    ...
    Public Sub New(ByVal add3ware As Boolean)
        If add3ware Then
            m_type = DeviceType.Threeware
            m_location = "/dev/tw_cli/clip"
            m_isoptical = False
            m_dontsavelog = True
    ...
    Private Sub GetDeviceInfo()
        If m_output.Length > 0 Then
            ...
            m_form = GetInfo("Form Factor").ToString.Replace(" inches", """")
    ...
    Private Sub Reliability()
        ...
        Dim id196 As String = m_newvitals.ReallEvent
        ...
        If Not IsNothing(id196) Then
            r_196 = Math.Round(id196 / My.Settings.Tune196 * 100)
            If System.Double.IsNaN(r_196) Then r_196 = 0
            If r_196 > 100 Then r_196 = 100
            m_reliability.ReallocationEvents = Math.Round(5 - r_196 / 20)
            If m_reliability.ReallocationEvents < 1 Then m_reliability.ReallocationEvents = 1
        Else
            r_196 = 0
            m_reliability.ReallocationEvents = Nothing
            divider -= 28
        End If
        ...
        If divider > 0 Then
            m_reliability.Overall = Math.Round(5 - (r_5 * 2.1 + r_196 * 1.4 + r_197 * 1.6 + r_198 * 3.9) / divider)
            If m_reliability.Overall < 1 Then m_reliability.Overall = 1
        Else
            m_reliability.Overall = 0
        End If
    ...
    Public Sub RunTest(ByVal test As TestType)
        ...
        Select Case test
            ...
            Case TestType.Offline
                ...
                Dim s As Date = Now
                Dim t As Long = ConvertToSeconds(m_pollingtimes.OfflineData)
            ...
            Case TestType.ShortTest
                ...
                Dim s As Date = Now
                Dim t As Long = ConvertToSeconds(m_pollingtimes.ShortTest)
                Dim e As Date = s.AddSeconds(t)
                m_testtiming.Start = s
                m_testtiming.Duration = t
                m_testtiming.Conclusion = e
            Case TestType.LongTest
                ...
                Dim s As Date = Now
                Dim t As Long = ConvertToSeconds(m_pollingtimes.ExtendedTest)
                Dim e As Date = s.AddSeconds(t)
                m_testtiming.Start = s
                m_testtiming.Duration = t
                m_testtiming.Conclusion = e
            Case TestType.Conveyance
                ...
                Dim s As Date = Now
                Dim t As Long = ConvertToSeconds(m_pollingtimes.ConveyanceTest)
                Dim e As Date = s.AddSeconds(t)
                m_testtiming.Start = s
                m_testtiming.Duration = t
                m_testtiming.Conclusion = e
            Case TestType.ScsiLongTest
                smartctl.SendCmd("-t long " & m_location)
                Dim s As Date = Now
                Dim t As Long = Val(m_testduration)
                Dim e As Date = s.AddSeconds(t)
                m_testtiming.Start = s
                m_testtiming.Duration = t
                m_testtiming.Conclusion = e
    ...
    Public ReadOnly Property Model As String
        Get
            If m_type = DeviceType.Threeware And Not m_isvaliddevice Then Return "3ware clipboard"
    ...
    Public ReadOnly Property SmartSupport As Support
        Get
            If m_type = DeviceType.Threeware And Not m_isvaliddevice Then Return Support.Unavailable
    ...
    Public ReadOnly Property InDatabase As Boolean
        Get
            If m_type = DeviceType.Threeware And Not m_isvaliddevice Then Return False
            Return m_indatabase
        End Get
    End Property
    ...
    Public ReadOnly Property Health As Status
        Get
            If m_type = DeviceType.Threeware And Not m_isvaliddevice Then Return Status.Unkonwn
    ...
    Public ReadOnly Property Temperatures As Temperatures
        Get
        ...
        Get
            ...
            If Not m_isunavailable Then
                If IO.File.Exists(m_location) Then
                    value = IO.File.ReadAllText(m_location)
                Else
    ...
    Public Sub Update()
        ...
        If m_type = DeviceType.ATA Or m_type = DeviceType.USB Then
            ...
            m_output = smartctl.SendCmd("-a -g all -g wcreorder -P use -l scttemp " & cmdlineoptions.Replace(vbCrLf, "") & " " & m_location)
        ElseIf m_type = DeviceType.SCSI Then
            m_output = smartctl.SendCmd("-a -g all " & m_location)
        ElseIf m_type = DeviceType.Threeware Then
            m_output = smartctl.SendCmd("-a /dev/tw_cli/clip")
        Else
            If IO.File.Exists(m_location) Then
    ...
    Public Property DisplayTrayIcon As Boolean
        Get
            If m_type = DeviceType.Threeware Then
                Return False
            Else
        ...
        End Get
        Set(ByVal value As Boolean)
            If Not m_type = DeviceType.Threeware Then
                If Not IsNothing(m_devtrayicon) Then
                    m_devtrayicon.Visible = value
                    If value = True Then UpdateTrayIcon()
    ...
    Public Sub UpdateTrayIcon()
        ...
        If m_type = DeviceType.Virtual Or m_type = DeviceType.Threeware Then Exit Sub
...
Public Structure Ratings
    Dim Overall, _
        OfflineUncorrectable, ReallocationCount, ReallocationEvents, CurrentPending, _
