            ...
            If My.Settings.InfoPanel Then
                If Main.WindowState = FormWindowState.Minimized Then
                    ...
                    If TrayPanel.Visible Then
                        Exit Sub
                    Else
                        TrayPanel.SetPanel(m_model, m_location, m_rating, m_temperature, m_remaininglife, m_health, m_type)
                        TrayPanel.Show(Main)
                    End If
                End If
            Else
...
#End Region
    ...
    Private m_badsectors As String
    ...
    Private Sub GetSmartctlVersion()
        'first row output samples:
        ...
        If My.User.IsInRole(ApplicationServices.BuiltInRole.Administrator) Then
            i = GetIndexOf("START OF INFORMATION SECTION") - 3
        Else
            i = GetIndexOf("START OF INFORMATION SECTION") - 4
        End If

        If i < 0 Then
            m_smartctl = "N/A"
        Else
            Dim ver() As String = m_output(i).Split(" ", 4, StringSplitOptions.RemoveEmptyEntries)
            If IsNumeric(ver(1)) Then
                'the second item is a number, like "5.39" in the second sample and not a word,
                'like "version" in the first sample
                m_smartctl = ver(1)
            Else
                'the second item is NOT a number, like "5.39" in the second sample, but a word,
                'like "version" in the first sample; in this case the version # is the third item
                m_smartctl = ver(2)
            End If
    ...
    Public ReadOnly Property BadSectorsCount As String
        Get
            If m_smartenabled <> Support.Enabled Then m_badsectors = "N/A"
    ...
    Public Sub Update()
        ...
        If m_type = DeviceType.ATA Or m_type = DeviceType.USB Then
            If My.Settings.Tolerance Then m_options += My.Settings.ToleranceLevel
            If My.Settings.PowerMode Then m_options += My.Settings.PowerModeLevel

            m_output = smartctl.SendCmd("-a -g all -g wcreorder -P use " & m_options.Replace(vbCrLf, "") & " " & m_location)
        ...
        'If GetIndexOf("START OF INFORMATION SECTION") > 0 Then
        ...
        If m_isvaliddevice Then
            ...
            'SaveVitals()
            'If IsNothing(m_oldvitals) Then LoadVitals()
            CompareVitals()
